...
function linerLoboScrollToTop() {
	$('#liner-panel-lobo').scrollTo($('#liner-table-of-contents-lobo'),500,{offset: {top:-5,left:-50}});
}

function linerLoboScroll(where) {
	$('#liner-panel-lobo').scrollTo(
		$(where),500,{offset: {top:-5,left:-50}}
	);
}
...
function calculateTabContentHeight(selectedItem) {
	...
	// If taller than 700px, set doSwipe to true if width is less than 768px and do other things if more than 768px
	else {
		...
		if (($(window).width() >= 768) && ($(window).width() < 1082)) {
// 				console.log("Getting here?");
			...
			// console.log(doSwipe + " for 768 <= width < 960 calculateTabContentHeight()");
			}
		
// 		console.log(doSwipe + " at end of calculateTabContentHeight()");
...
// 	console.log("initialiseHammer() has been called");
}
...
function lauchLearnFromURL(URLstring) {
	...
	if (matchedID == "dog") {
		...
		switch (URLstring) {
			case "LSRoad2017":
				launchStream("Road201718");
				break;
				
			case "LSLiturgy2017":
				launchStream("Liturgy201718");
				break;
				
			case "LSLondon2017":
				launchStream("London201718");
				break;
...
jQuery(document).ready(function($) {
    ...
    // 	console.log(doSwipe + " just before initialiseTabs() is called");
...
// 		console.log(doSwipe + " just before initialiseHammer() is called");
